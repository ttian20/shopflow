...
<<<<<<< HEAD
        ...
        $this->get_result = $this->handler->get($this->options['prefix'].$sessID);
        //$data = msgpack_unpack($this->get_result);
        $data = json_decode($this->get_result, true);
        //$_SESSION = $data;

        return $this->serializes($data);
        //return $this->get_result;
=======
		$this->get_result = $this->handler->get($this->options['prefix'].$sessID);
        return $this->get_result;
>>>>>>> 28249b49a33502919e3cfacd9507e8980fb96b55
	}
...
<<<<<<< HEAD
        if($sessData){
            $res = $this->unserializes($sessData);
            //$sessData = msgpack_pack($res);
            $sessData = json_encode($res);
        }

        //$sessData = msgpack_unpack($pack_res);
=======
>>>>>>> 28249b49a33502919e3cfacd9507e8980fb96b55
...
<<<<<<< HEAD
    private function unserializes($data_value) {
    ...
    private function serializes($data_value) {
        $str = '';
        if($this->option['prefix']){
            $str .= $this->option['prefix'] . '|' . serialize($data_value);
        }else{
            $arr = array();
            foreach($data_value as $dk => $dv){
                $arr[] = $dk .'|' . serialize ($dv);
            }
